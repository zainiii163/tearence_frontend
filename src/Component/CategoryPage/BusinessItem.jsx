...
const BusinessItem = memo(({ item, viewMode = "grid", onUpsellClick }) => {
  ...
  const formatRevenue = (revenue) => {
    if (!revenue) return "Revenue not disclosed";
    const symbol = "$";
    if (revenue >= 1000000) {
      return `${symbol}${(revenue / 1000000).toFixed(1)}M`;
    } else if (revenue >= 1000) {
      return `${symbol}${(revenue / 1000).toFixed(0)}K`;
    }
    return `${symbol}${revenue.toLocaleString()}`;
  };
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
              ...
              {item.annual_revenue && (
                <div className="flex items-center gap-1 text-green-600 font-semibold">
                  ...
                  <span>{formatRevenue(item.annual_revenue)}</span>
                </div>
              )}
              {item.employees && (
              ...
              {item.established && (
                <div className="flex items-center gap-1 text-muted-foreground">
                  ...
                  <span>{item.established} years</span>
                </div>
              )}
            </div>
        ...
        {showUpsellModal && onUpsellClick && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
              <h3 className="text-lg font-semibold mb-4">Promote Your Business</h3>
              <p className="text-gray-600 mb-4">Choose how you want to promote your business listing:</p>
        ...
        <div className="space-y-2">
          ...
          {item.annual_revenue && (
            <div className="flex items-center gap-1 text-sm font-semibold text-green-600">
              <FaChartLine className="h-4 w-4" />
              <span>{formatRevenue(item.annual_revenue)} revenue</span>
            </div>
          )}
          {item.location && (
            <div className="flex items-center gap-1 text-sm text-muted-foreground">
