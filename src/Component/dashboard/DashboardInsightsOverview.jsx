...
const DashboardInsightsOverview = ({ accountHint = 'personal' }) => {
  ...
  const isBusiness = (data.account_type || accountHint) === 'business';
  const cards = [
    {
      label: isBusiness ? 'Completed sales' : 'Items sold',
      value: data.sales?.sold_items ?? 0,
      icon: FaShoppingBag,
      tone: 'bg-emerald-600',
    },
    {
      label: isBusiness ? 'Net seller revenue' : 'Seller revenue',
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
          <div className="mb-3 flex items-center justify-between gap-2">
            ...
            <Link to="/dashboard?tab=notifications" className="text-xs font-semibold text-slate-600 hover:underline">
              View all
            </Link>
          </div>
          {recent.length === 0 ? (
