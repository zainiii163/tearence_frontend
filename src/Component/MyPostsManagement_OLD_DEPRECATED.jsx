...
const MyPostsManagement = () => {
  ...
  // Combine all posts
  const allPosts = [
    ...(Array.isArray(myAffiliateList) ? myAffiliateList : (myAffiliateList?.data || [])).map(affiliate => ({ ...affiliate, type: 'affiliate' })),
  ...
  const stats = {
    ...
    affiliate: Array.isArray(myAffiliateList) ? myAffiliateList.length : (myAffiliateList?.data?.length || 0),
