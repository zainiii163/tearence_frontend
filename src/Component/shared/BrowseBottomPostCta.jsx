...
/**
 * Bottom post CTA — one long bar with a single action button (no marketing title).
 */
const BrowseBottomPostCta = ({
  ...
  onPostClick,
  theme = 'purple',
  /** @deprecated always button-in-bar; kept for call-site compatibility */
  buttonOnly: _buttonOnly = true,
  compact = false,
}) => {
  const themes = {
    purple: {
  ...
  if (isSlate) {
    return (
      <section className="mt-8 mb-2">
        <div className="property-cta-band flex justify-center items-center py-5 px-4">
          <button
            type="button"
            onClick={onPostClick}
            className="inline-flex items-center justify-center px-8 py-3 text-sm font-semibold tracking-wide uppercase bg-[var(--prop-copper)] hover:bg-[var(--prop-copper-deep)] text-white transition-colors rounded-lg"
          >
            {label}
          </button>
        </div>
  ...
  return (
    <section className="mt-8 mb-2">
      <div
        className={`bg-gradient-to-r ${t.gradient} rounded-2xl px-6 py-5 sm:px-8 sm:py-5 text-center shadow-lg`}
        role="presentation"
      >
        <button
          type="button"
